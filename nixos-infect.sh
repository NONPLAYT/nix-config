...
#
# Usage:
#   curl -sL https://raw.githubusercontent.com/NONPLAYT/nix-config/main/nixos-infect.sh | NIX_HOST=hostname.example.com bash
#
# Environment:
#   NIX_HOST      target host name — accepts FQDN (default: $(hostname -f))
#   REPO_URL      nix-config repo to clone (default: NONPLAYT/nix-config)
#   REPO_BRANCH   branch to check out (default: master)
...
REPO_URL="${REPO_URL:-https://github.com/NONPLAYT/nix-config.git}"
REPO_BRANCH="${REPO_BRANCH:-master}"
NIX_CHANNEL="${NIX_CHANNEL:-nixos-25.11}"
GEN_DIR="/etc/nixos-generated"
REPO_DIR="/etc/nixos-repo-staging"

DEFAULT_KEY="ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIM2y5ojFwo0p78rZgc3S31h7CyIdCyWOO9xcajs51m7F bxteam.org"
...
checkEnv() {
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        ...
        if req apt-get; then
            apt-get update -y && apt-get install -y curl bzip2 xz-utils tar iproute2 gawk coreutils git passwd
        elif req dnf; then
            dnf install -y curl bzip2 xz tar iproute gawk coreutils git shadow-utils
        elif req yum; then
            yum install -y curl bzip2 xz tar iproute gawk coreutils git shadow-utils
        else
...
networkCfg() {
    ...
    [ -n "$eth0_name" ] || {
        warn "no network interface detected — skipping networking.nix"
        return 1
    }
...
scaffoldRepo() {
    [ -n "${NO_REPO:-}" ] && {
        log "NO_REPO set — skipping repo scaffold"
        return 0
    }
