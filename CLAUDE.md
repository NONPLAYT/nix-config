...
- stockholm's HTTP names are published through one cloudflared tunnel
  (`server/services/cloudflared`): `repo` and `mail` are CNAMEs to it and
  ...
  roundcube is php-fpm. reposilite is a plain HTTP upstream, so for it the
  tunnel *is* the reverse proxy.
...
- `stockholm.bxteam.org` is the only name still pointing at the box, so
  everything the tunnel cannot carry hangs off it: the MX with imap and
  submission (`mailserver.fqdn`, which is also the HELO name -- the provider's
  PTR has to match it), reality's cover site and the xctrl agent API. Its ACME
  certificate is the one postfix and dovecot serve.
